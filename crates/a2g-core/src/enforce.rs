...
mod tests {
    ...
    /// Minimal no-op ledger for unit tests inside a2g-core.
    /// The tests here focus on the enforcement logic; the ledger
    /// path (revocation + rate-limit) is covered by integration tests
    /// that run against the real SQLite implementation.
    struct TestLedger;
